...
if ((${#missing[@]} > 0)); then
  ...
  echo "Re-run from repo root with: bash scripts/ci/install-git-hooks.sh" >&2
  exit 1
fi

echo "✅ Git hooks installed:"
echo "   pre-commit  → essential checks on staged files (hygiene, secrets, lint, types, …)"
echo "   pre-push    → fast gates (./scripts/ci/pre-push-gates.sh — lint, tests, build, gitleaks)"
echo ""
echo "Full CI mirror before PR (coverage, pip-audit, npm audit): ./scripts/ci/quality-gates.sh"
