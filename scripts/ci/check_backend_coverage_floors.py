#!/usr/bin/env python3
"""Enforce full backend coverage floors (server/ + cli/) — DECISIONS #142.

coverage.py only gates a single combined total via ``fail_under``. This script
...
Integration-only paths (Postgres/Mongo adapters, embedders, server/main.py) are
excluded from the unit tier and bring full-backend coverage below FE thresholds.
Floors load from ``[tool.rag_params_finder.backend_coverage_thresholds]`` in
``pyproject.toml``; otherwise the baseline defaults above apply.

Usage:
  python scripts/ci/check_backend_coverage_floors.py [.reports/coverage/backend-full.json]
"""
...
def load_floors(pyproject: Path | None = None) -> dict[str, float]:
    """Load full-backend statement/branch/line floors from pyproject.toml."""
    ...
    section = (
        data.get("tool", {}).get("rag_params_finder", {}).get("backend_coverage_thresholds", {})
    )
    for key in floors:
        if key in section:
...
def main(argv: list[str]) -> int:
    report = Path(argv[1] if len(argv) > 1 else ".reports/coverage/backend-full.json")
    ...
    measured = metrics_from_totals(totals)
    print(
        "Backend coverage floors (full server/ + cli/, unit tier; functions n/a): "
    ...
    if failures:
        print("FAIL — below backend floors:", file=sys.stderr)
        for line in failures:
            print(f"  {line}", file=sys.stderr)
        return 1

    print(
        f"PASS — statements/branches/lines meet "
        f"{floors['statements']:.0f}/{floors['branches']:.0f}/{floors['lines']:.0f}"
    )
    return 0
