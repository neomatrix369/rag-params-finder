...
- **Frontend coverage gate** (Slice 44 Must Phase A+B) — `quality-gates.sh` / `pre-push-gates.sh` run `npm run test:coverage`; CI frontend runs `test:ci`; Vitest v8 thresholds statements/functions/lines ≥95%, branches ≥90% with `all: true` (DECISIONS #139) (**VERIFIED**).
