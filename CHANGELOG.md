...
- **ADR-004 — Dual-backend storage** — Accept PostgreSQL/pgvector (including Supabase) as a first-class backend **alongside** MongoDB Atlas; [ADR-003](docs/adr/ADR-003-mongodb-atlas-vector-store.md) superseded. Default remains `mongodb` until an **explicit** flip decision (DECISIONS #129); local comparison evidence in [slice-38-quality-comparison.md](docs/plan/gate-evidence/slice-38-quality-comparison.md) (latency PASS; rank overlap informational). See [ADR-004](docs/adr/ADR-004-postgresql-pgvector-vector-store.md).
- **Slice 38 local dual-backend comparison** — Mirrored 120-run grids on `mongodb-local` + `postgres-local` (`example-*-local-parallel`); QUERYING `elapsed_ms` median/max ≤2× **PASS**; top-1/3/5 overlap recorded as operator evidence under independent backends (#129). Artifact: [slice-38-quality-comparison.md](docs/plan/gate-evidence/slice-38-quality-comparison.md).
