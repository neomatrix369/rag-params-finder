...
- **Project-hygiene uplift (2026-09-24)** — Semgrep SAST and OSV-Scanner SCA now run in `nightly.yml` with the same rule packs and waivers as `security-scan.sh` (new `osv-scanner.toml` mirrors `pip-audit.sh`; local Semgrep no longer uses `--config=auto`); `vulture` dead-code gate (`[tool.vulture]`) in pre-commit, `ci.yml` and `quality-gates.sh`; frontend cognitive complexity (`eslint-plugin-sonarjs`, ≤15) added to the complexity report (reporting, 5 functions over); nightly knip + jscpd frontend trend (report only, duplication baseline 6.2%). Fixed GitHub Actions shell-injection pattern in the `ci.yml` gitleaks steps (event values via `env`). Dockerfiles still run as root — documented `nosemgrep` waivers pending an owner decision (**IMPLEMENTED**; Semgrep full repo 0 findings, vulture 0, 472 unit tests).
...
- **Postgres index preflight + four-value `storage_mode`** (Slice 36) — experiment submit on `STORAGE_BACKEND=postgres` now verifies the `vector` extension and the `chunks_embedding_384_hnsw` / `chunks_embedding_1024_hnsw` / `chunks_text_search_gin` indexes by catalog introspection and returns **HTTP 422** naming what is missing (no Atlas Admin API calls, no quota/reconcile step); `rag-params-finder indexes list` prints PRESENT/MISSING on Postgres while `indexes reset` stays Atlas-only; `/healthz`, `/health`, and db-stats report `storage_mode` ∈ `mongodb-local` \| `mongodb-cloud` \| `postgres-local` \| `postgres-cloud` — the mode string no longer says `supabase` or `local-postgres`. See [troubleshooting.md → Postgres index preflight failed](docs/user-guide/troubleshooting.md#-postgres-index-preflight-failed)
...
- **Bayesian sweep strategy** (Slice 41A) — `execution.search_strategy: bayesian` activates Optuna TPE to tune `chunk_size` and `overlap` automatically; CLI prints a Trial History table with per-trial chunk/overlap/state/score; `bayesian_summary.trial_log` returned by the experiments API; `configs/mongodb/example-local-bayesian.yaml` and `configs/mongodb/example-unified-retrievers-bayesian.yaml` provided as activation examples.
...
- **Slice specs → numbered theme folders** (Slice 40 Should, #162) — `docs/plan/slices/0N-<theme>/SLICE-*.md` by delivery-wave order (`01-core-pipeline` … `07-quality-craft`); [`PROGRESS.md`](docs/plan/slices/PROGRESS.md) stays flat status SSOT; theme index at [`docs/plan/slices/README.md`](docs/plan/slices/README.md). Gate evidence remains flat under `docs/plan/gate-evidence/`.
...
- **Slice theme-folder link depth** — relative links from `docs/plan/slices/0N-<theme>/` and flat `PROGRESS.md` to guides, ADRs, PRD/TRAIL, gate-evidence, `PROGRESS`, and `server/db/postgres/schema.sql`; stale flat paths in `.env.example` and DECISIONS #133; living `schema.sql` hrefs under user/contributor guides.
- **Unit tier without a database URI** — experiment detail no longer resolves a storage backend when run rows are already present, and Mongo `store_factory` tests supply a URI, so the unit suite passes with `MONGODB_URI` unset (CI) as well as with a populated `.env` (local)
