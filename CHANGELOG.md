...
- **Local/cloud parity + low-friction switching** (Slice 37) — four-flag `./start-services.sh --{mongodb|postgres}-{local|cloud}` (legacy `--local`/`--postgres` aliases); bare start respects `.env` `STORAGE_BACKEND`; compose profiles `mongodb-local`/`postgres-local` (aliases `local-atlas`/`local-postgres`); `database_provider: supabase` → `postgres` + warning; `vector_db_id` = `storage_mode:<host>`; `POST /experiments` rejects config↔server engine mismatch with HTTP 422 **before** index/SIE preflight and persists `storage_mode`; postgres lifecycle `start|stop|reset|status`; Path B Session-mode / pause runbook. See [postgres-setup.md](docs/user-guide/postgres-setup.md), [configuration.md → Engine × Location](docs/user-guide/configuration.md#-environment-variables-env), [troubleshooting → Config engine mismatch](docs/user-guide/troubleshooting.md#-config-engine-mismatch-database_provider--storage_backend)
...
- **Postgres/Supabase operator docs parity** — `postgres-setup.md` mirrors Mongo/SIE scaffolding (env tables, Path B, diagnostics); Supabase clarified as hosted Postgres under `STORAGE_BACKEND=postgres` + `DATABASE_URL` (no `SUPABASE_URI`); empty-state dashboard offers both `configs/mongodb/` and `configs/supabase/` examples.
