...
./scripts/local-atlas.sh [start|stop|reset|status]  # manage local Atlas container standalone
...
Host CLI unchanged: `SERVER_URL=http://localhost:8001`. See `docs/slices/SLICE-14-DOCKER-COMPOSE.md` and `docs/user-guide/local-atlas-setup.md`.
...
| `scripts/local-atlas.sh` | Manage `mongodb-atlas-local` container: `start\|stop\|reset\|status`; prints CLI connection string |
| `docker-compose.local-atlas.yml` | Overlay: server `MONGODB_URI` → `mongodb-local:27017`; activate with `./start-services.sh --local` |
