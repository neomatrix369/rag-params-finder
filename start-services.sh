...
# Usage: ./start-services.sh [--force-build] [--local]
#   Rebuilds images only when build context changed, images are missing, or --force-build.
#   Env: RAG_FORCE_BUILD=1 (same as --force-build), RAG_DEV_STACK=1, RAG_LOCAL_ATLAS=1, NONINTERACTIVE=1
set -e
set -o pipefail
...
Backends:
  ...
  Local (--local):  starts mongodb-atlas-local container; no .env MONGODB_URI needed
                    CLI: export MONGODB_URI="$LOCAL_URI"
                    Manage container standalone: ./scripts/local-atlas.sh
EOF
}
...
if [[ "$LOCAL_ATLAS" == "1" ]]; then
  COMPOSE_FILES+=(-f docker-compose.local-atlas.yml)
...
ensure_env() {
  ...
  # Local Atlas: server MONGODB_URI is set by docker-compose.local-atlas.yml overlay.
...
print_unhealthy_server_hint() {
  ...
  if [[ "$LOCAL_ATLAS" == "1" ]]; then
    ...
    echo "  ./scripts/local-atlas.sh status"
  else
...
if [[ "$LOCAL_ATLAS" == "1" ]]; then
  ...
  echo "Manage MongoDB only:   ./scripts/local-atlas.sh [start|stop|reset|status]"
  echo "Reset all data:        docker compose -f docker-compose.yml --profile local-atlas down -v"
else
  echo ""
