...
>
> **2026-09-24 note (plan Add — docs only):** New embedding-provider track **48A/48B/48C/48D** (DoubleWord Qwen3-Embedding-8B, **batch-first**, docextract async pattern) from the owner brief [`BRIEF-doubleword-embedder.md`](BRIEF-doubleword-embedder.md); specs under [`slices/08-embedding-providers/`](slices/08-embedding-providers/). Start at **48A** (T0 spike needs `DOUBLEWORD_API_KEY`; V3 embeddings-batch is a hard gate; S1 mixed-provider axis needs no key). Owner decided 2026-09-24: completion window **1h**, pricing per window (#205). 48C D1 decided (#207); **2026-09-25:** D2/D3 reopened by review (#236, #237) and re-decided by the owner: `embedding_512` migration before 48C (#238), boot-time identity migration (#239). ADR-005 created (Proposed); `defer_for_pre_embed` → `defer_async_embeddings` (#234). No other open HITL items at planning level; slice-start checks remain (T0 spikes). Nw re-review iteration 2 remediated (#206). Post-merge EFP health check + quality lens re-run 17/18 (#209–#210); DoubleWord catalog has one embedding model (#208); owner: 48B stays Must, next wave = 48A T0 spike first (#212). See DECISIONS #188–#212.
>
> **2026-09-24 note (plan Adds — docs only):** Elasticsearch vector-store track **49 → 50 → 51** (specs under [`slices/05-storage/`](slices/05-storage/), ADR-006, DECISIONS #213–#219) and Redis track **52 → 53** (+ **54** Could) from [`BRIEF-redis-evaluation.md`](BRIEF-redis-evaluation.md) (ADR-007, DECISIONS #220–#229). Slice 52 is a docs-only evaluation spike that can start now; its owner GO / NO-GO gates 53 and 54. MoSCoW confirmed by the owner (#233): 52 Must, 53 Should → Must on Branch A GO, 54 Could.
>
> **2026-09-25 note (Redis walkthrough, docs only):** [`_internal/STORE-E2E-WALKTHROUGH.md`](../_internal/STORE-E2E-WALKTHROUGH.md) traces all four stores. New 49B/51/53 scenarios close its gaps, 52 reuses it in R0, and 50–54 list the official vendor docs to load at slice start. DECISIONS #253–#254.
