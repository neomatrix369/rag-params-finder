...
| Hosted `postgres-cloud` production-claim quality/latency matrix + PRD bookkeeping + post-flip smoke + sync-docs parked from 38 | **This slice — residuals below** | DECISIONS #125 / #126 |
...
Anything that was **not a clear Yes** on the Slice 38 realism review lives here. Slice 38 COMPLETE gates only on local comparison + ADR + independent dual-backend model (#129) + `slice-38.json` + tracker/CHANGELOG. Slice 43 status stays ✅; these are post-COMPLETE residuals.

| Item | Why not 100% Yes on 38 | Acceptance (when picked up) |
|---|---|---|
| Hosted `postgres-cloud` quality + latency matrix vs Mongo | Production claim ≠ local flip; free-tier pause risk | Amend comparison artifact with hosted mode scope; QUERYING ≤2×; numeric top-3; claim only if PASS/CONDITIONAL |
| Slice 37 hosted Path B as Before for *any* flip | Nuanced — required for production claim only | Use `slice-37.json` hosted smoke as Before for this residual |
| Hosted-unreachable / withhold-production-claim GWT | Belongs with hosted matrix | Document partial hosted scope; no PASS claim |
| Post-flip fresh-clone → `storage_backend=postgres` without `<project-ref>` | Only applies after flip; N/A if deferred | Smoke: copy `.env.example`, start, clear remediation or `--postgres-local` guidance; `/healthz` reports postgres |
| Post-flip Mongo rollback via `--mongodb-*` + canonical `mongodb` docs | Same — flip-gated | Two-command Mongo path works; docs say `mongodb` not bare `mongo` |
| ADR **Pro-tier / non-pausing** note as COMPLETE mandate | Production-claim prose, not local cutover | ADR-004 Consequences (or amend) state Pro/non-pausing for warm demos |
...
| `/sync-docs` full operator + contributor footprint after cutover | “Yes if flip; else partial” — not unconditional | Run `/sync-docs` for README/default, mongodb↔postgres cross-links, architecture, CLAUDE Key Files |
...
| Separate “specification coverage” After-Check for every GWT | Probes already required inside comparison artifact | Ensure post-flip GWT above have probes when those residuals run |
| Graphiti cutover-decision episode as COMPLETE gate | Process hygiene | Optional episode `rag-params-finder-flow-planner` |
