...
**Current** (2026-09-24 Add): new embedding-provider track **48A → 48B → 48C** (+48D Could; DoubleWord Qwen3-Embedding-8B, batch-first; brief [`BRIEF-doubleword-embedder.md`](../BRIEF-doubleword-embedder.md); DECISIONS #188–#212, reviewer fixes #234–#237, owner D2/D3 decisions #238–#239, ADR-005 Proposed 2026-09-25) — 48A is the next startable Must, starting with its T0 spike (#212), alongside **28** (external). Also added 2026-09-24 (docs only): Elasticsearch vector-store track **49A → 49B → 50 → 51** (#213–#219; restructured 2026-09-25 after an end-to-end desk-check, #240–#249) and Redis track **52 → 53** (+54 Could; brief [`BRIEF-redis-evaluation.md`](../BRIEF-redis-evaluation.md); #220–#229). **52** is docs-only and may start now; **53** waits on 49A/49B/50/51 ✅ + 52 GO. 2026-09-25: the four-store walkthrough's gaps are assigned to 49B/51, and 50–54 carry External references (#253–#254). Previous: Next Must **28** (external). **22** ✅ · **40** ✅ · **45** ✅ · **44** ✅ Residual §4 **IMPLEMENTED** (#163; Nightly artifact VERIFIED pending). Migration track: **32/33/34–38 code ✅ on main**; formal gate-closure debt now scoped to **32C** → **32B** only (coverage/mutation/nw-review + gate-evidence schema — see reconcile 2026-09-23, DECISIONS #186–#187). Then **41B**. Deferred Mongo QoL **26/27/19**. Genuinely-open craft: **46** (backend cov 80–85%), **47** (xenon B/A/A).
...
| 46 — Backend coverage 70.1% → 80–85% | 📋 PLANNED | ~3–4 h | FastAPI TestClient (server/api), Typer CliRunner (cli/), orchestrator branching (server/core/pipeline). ATDD format. Floor raise to 80 (fail_under + 78/65/78 per-metric). — [`SLICE-46-BACKEND-COVERAGE-85.md`](07-quality-craft/SLICE-46-BACKEND-COVERAGE-85.md) |
| 47 — Xenon complexity E/C/C → B/A/A | 📋 PLANNED | ~2–3 h | Fix 2 E-rank (orchestrator CC=40, experiments_lifecycle CC=32) + D/C blocks. 3-phase tighten. Gate already wired at E/C/C. — [`SLICE-47-COMPLEXITY-TIGHTEN.md`](07-quality-craft/SLICE-47-COMPLEXITY-TIGHTEN.md) |
| 48A — Mixed-provider axis + DoubleWord batch provider | 📋 PLANNED | ~9–11 h | **Must** — batch-first (#202): provider derived per model; openai SDK batch client ported from docextract; plan+submit-all then **release the single sweep worker**; detached asyncio watcher polls all batches, fills SQLite cache, schedules runs; checkpoint + boot resume; unavailable-model registry; supervised watcher + `/healthz` status; 422 without key; window default **1h** (#205); S1 may ship first as its own PR, S2–S4 ship together — [`SLICE-48A-DOUBLEWORD-BATCH-PROVIDER.md`](08-embedding-providers/SLICE-48A-DOUBLEWORD-BATCH-PROVIDER.md) |
| 48B — DoubleWord cost + hardening | 📋 PLANNED | ~5–6 h | **Must** — per-run/per-experiment cost (analytics → token fallback); double-billing guard (adopt by `metadata.job_key`); one batch shared by experiments; progress card; ctx-limit diagnostics; per-window pricing (#205); moves ADR-005 Proposed → Accepted at slice close — [`SLICE-48B-DOUBLEWORD-COST-HARDENING.md`](08-embedding-providers/SLICE-48B-DOUBLEWORD-COST-HARDENING.md) |
| 48C — MRL dims + query-instruction axes | 📋 PLANNED | ~4–6 h | **Should** — one paid pass at max dim + truncate; instruction variants reuse doc vectors; D1 decided (#207): per-model sub-axes under `embedding.axes`; D2/D3 re-decided 2026-09-25: add `embedding_512` column in a pre-48C PR (#238); composite `model#d<dim>` identity with a boot-time migration of older records (#239) — [`SLICE-48C-DOUBLEWORD-MRL-INSTRUCTION-AXES.md`](08-embedding-providers/SLICE-48C-DOUBLEWORD-MRL-INSTRUCTION-AXES.md) |
| 48D — DoubleWord realtime mode | 📋 PLANNED | ~2 h | **Could** — optional; promoted to Must only if 48A spike V3 fails — [`SLICE-48D-DOUBLEWORD-REALTIME-MODE.md`](08-embedding-providers/SLICE-48D-DOUBLEWORD-REALTIME-MODE.md) |
| 49A — Vector-store port + registry seam | 📋 PLANNED | ~4–6 h | **Must** — `VectorStore` port + `VectorCapabilities` + registry mapping to composite `MongoVectorStore`/`PostgresVectorStore`; `get_vector_store()`; `get_retriever_backend()` resolves via the vector store (same signature); `VECTOR_STORE_BACKEND` locked equal to `STORAGE_BACKEND`; one `DatabaseProvider` Literal; characterization tests first + AST store-string guard (`server/` + `cli/`). **No ES code, zero behaviour change.** Split from 49 after the round-3 desk-check (#240/#242) — [`SLICE-49-VECTOR-STORE-PORT-SPLIT-REGISTRY.md`](05-storage/SLICE-49-VECTOR-STORE-PORT-SPLIT-REGISTRY.md) |
| 49B — Split-store data-path rewire | 📋 PLANNED | ~6–8 h | **Must** — moves chunk write/delete/stats/preflight/boot onto the vector port (10 sites); delete vectors first + idempotent (#246); `/healthz` 503 if either store is down, adds `run_state_mode` + `stores{}` (#247); fail-fast boot; in-memory split-store E2E test that fails on 49A; `_patch_backends` + anti test-theater guard; pairing rule (ii) (#241) — [`SLICE-49B-VECTOR-STORE-DATA-PATH-REWIRE.md`](05-storage/SLICE-49B-VECTOR-STORE-DATA-PATH-REWIRE.md) |
| 50 — Elasticsearch adapter core | 📋 PLANNED | ~6–8 h | **Must** — `server/db/elasticsearch/` + retrieval; single `rpf-chunks` index, per-dim fields (D4), explicit unquantized HNSW, `(1+cos)/2` scale, mandatory `embedding_model`/`experiment_id`/`run_id` filter; dense knn + BM25 + client-side RRF (D3, shared fusion); refresh-before-return; `[elasticsearch]` extra (lazy). 95% retrieval coverage gate. — [`SLICE-50-ELASTICSEARCH-ADAPTER-CORE.md`](05-storage/SLICE-50-ELASTICSEARCH-ADAPTER-CORE.md) |
| 51 — Elasticsearch closeout (operability + CI + docs + ADR-006) | 📋 PLANNED | ~11–15 h | **Must** — **merged 51+52 (#218)**. Compose `elasticsearch-local` (Basic licence, security off, 1 GB heap, 127.0.0.1); `--elasticsearch-local/-cloud` + `elasticsearch start/stop/reset/status` (D5 pairs run-state store); `configs/elasticsearch/*` (9 basenames); `GET /api/stores` (fixes CLI thin-client leak); registry-driven FE labels; drift fixes (supabase provider, stop-services, health-check). **＋** nightly `elasticsearch-integration` job; 15-stage journey docs (`elasticsearch-setup.md` + QUICKSTART Path E + 8 more); registry-driven docs-parity gate in PR CI; `ADR-006`; cross-backend comparability (same YAML on Mongo/Postgres/ES). Optional 51a/51b PR split. — [`SLICE-51-ELASTICSEARCH-OPERABILITY-CI-DOCS-ADR.md`](05-storage/SLICE-51-ELASTICSEARCH-OPERABILITY-CI-DOCS-ADR.md) |
| 52 — Redis evaluation spike + ADR-007 (Proposed) | 📋 PLANNED | ~6–8 h | **Must** (owner-confirmed #233) — docs-only; fuses all research steps of the brief; reuses 49/51 review + journey (Redis delta only); throwaway PoC on `redis:8` vs valkey-search (per-dim fields, filtered KNN, BM25, `FT.HYBRID` probe, `1 − d/2` score check, memory per vector, eviction, AOF); Branch B verdicts anchored to code; weighted scoring 50/30/20 + sensitivity; report + self-contained HTML guide; **owner GO/NO-GO** gates 53/54 — [`SLICE-52-REDIS-EVALUATION-SPIKE.md`](05-storage/SLICE-52-REDIS-EVALUATION-SPIKE.md) |
| 53 — Redis vector store: adapter + full journey | 📋 PLANNED | ~9–12 h | **Should (Must on Branch A GO)** (owner-confirmed #233) — `server/db/redis/` behind the 49 registry (+1 manifest line); reuses `rrf_fuse()`, `(1+cos)/2` (`1 − d/2`), `_CANDIDATES_MULTIPLIER`, 3-filter isolation, `stats_common`, capacity shape, contract suite + docs-parity gate **unchanged**; Redis-only preflight (Query Engine present, `noeviction`, `maxmemory` capacity); `redis-local` + AOF, `--redis-local` (D5), `configs/redis/*`, nightly, `redis-setup.md`, Path F, ADR-007 Accepted; **zero-changes diff guard**. Optional 53a/53b PR split — [`SLICE-53-REDIS-VECTOR-STORE-ADAPTER.md`](05-storage/SLICE-53-REDIS-VECTOR-STORE-ADAPTER.md) |
| 54 — Embedding-cache backend port + Redis cache | 📋 PLANNED | ~3–4 h | **Could** — `CacheBackend` (`get_many`/`put_many`) extracted from 48A's `embedding_cache.py`; SQLite stays default; Redis adapter under `rpf:emb:`; same tests parametrised; one-vs-two-instance eviction stance is HITL — [`SLICE-54-CACHE-BACKEND-PORT-REDIS.md`](08-embedding-providers/SLICE-54-CACHE-BACKEND-PORT-REDIS.md) |
...
| Date | Item | Outcome |
|------|------|---------|
| 2026-09-23 | Bash 3.2 empty-array fix (fix/storage-mode-bash32-empty-array) | `scripts/lib/storage_mode.sh:136` expanded empty `selected[@]` under `set -u` → 3 `test_storage_mode_resolve.py` failures on macOS `/bin/bash` 3.2 (CI Bash 5 unaffected; `start-services.sh` has no `-u`). Fixed with `${arr[@]+"${arr[@]}"}` guard + static regression test over `scripts/lib/*.sh` (runs on CI). **IMPLEMENTED**; **VERIFIED** 28/28 on Bash 3.2.57 + 5.3.3, unit tier 470 passed / 70.12% cov. |
...
| 2026-09-10 | Nightly #71 SCA (fix/nightly-sca-nltk-pypdf) | [Nightly run 34446337643](https://github.com/neomatrix369/rag-params-finder/actions/runs/34446337643) failed Dependency audit + Trivy container scan. Root cause: `nltk` 3.10.0 (many PYSEC/CVE), `pypdf` 6.15.0 (CVE-2026-84309/84310/84311), `pip` 26.1.2 (CVE-2026-13346), plus new transformers `CVE-2026-9856` (fix ≥5.10; blocked on ST&lt;4). Fixed: bump nltk/pypdf/pip; waive CVE-2026-9856 + nltk CVE-2026-81726 false-positive on 3.10.3 in `.trivyignore` / `.meterian` / `pip-audit.sh`. Local `pip-audit` **VERIFIED** exit 0. Nightly re-run **VERIFIED** pending merge. |
| 2026-08-07 | Full-backend coverage scope + project-hygiene tier (chore/project-hygiene-full) | **Coverage architecture**: switched from selective 4-module measurement at 95% to full `server/ + cli/` at honest baselines — combined 59.1% (`--cov-fail-under=59`); per-metric floors 61/47/61 (stmts/br/lines) in `[tool.rag_params_finder.backend_coverage_thresholds]`; `check_backend_coverage_floors.py` updated accordingly; all four enforcement points (ci.yml, nightly.yml, quality-gates.sh, pyproject.toml) aligned. **Hygiene tier**: complexity-report workflow (`complexity-report.yml`), `SECURITY.md`, `.github/CODEOWNERS`, `PULL_REQUEST_TEMPLATE.md`, `eslint.complexity.config.cjs`, `scripts/ci/complexity-report.sh`, `scripts/ci/write_complexity_summary.py`, nightly `nightly-tests-python`, `nightly-tests-node`, `nightly-complexity` jobs added. Complexity badge added to README. **IMPLEMENTED**; **VERIFIED** locally (347 pytest, 264 vitest, pre-push gates green). PR #— on `chore/project-hygiene-full`. |
| 2026-08-09 | Xenon complexity gate wired (chore/project-hygiene-full) | Hard gate added at baseline `E/C/C` (`xenon --max-absolute E --max-modules C --max-average C server/ cli/`) in quality-gates.sh, ci.yml, pre-push-gates.sh, nightly.yml. Passes current code. Target `B/A/A` blocked by 2 E-rank functions (orchestrator._run_sweep_inner CC=40, experiments_lifecycle._normalize_stale_running_status CC=32) + D/C blocks across 12 files — tracked in SLICE-47. **IMPLEMENTED**; **VERIFIED** locally (xenon exits 0). |
| 2026-08-09 | Slices 46/47 added (chore/project-hygiene-full) | Slice 46: backend coverage 70.1% → 80–85% (FastAPI TestClient for server/api, Typer CliRunner for cli/, orchestrator branching). Slice 47: xenon tighten E/C/C → B/A/A in 3 phases (Phase 1: fix E-rank; Phase 2: fix D-rank + module C; Phase 3: fix C-rank + module B). Specs in `docs/plan/slices/07-quality-craft/`. **PROPOSED**. |
| 2026-08-08 | pypdf CVE fix (chore/project-hygiene-full) | `pip-audit` pre-push gate blocked push: `pypdf 6.14.2` had CVE-2026-71852 + CVE-2026-71870 (fix: 6.15.0). Upgraded via `uv add "pypdf>=6.15.0"`. Commit `ae976bd`. **IMPLEMENTED**; push gate unblocked. |
| 2026-08-07 | ATDD test suite audit + gap analysis (chore/project-hygiene-full) | nw-acceptance-designer-reviewer produced requirements-based gap analysis (`docs/plan/gate-evidence/test-gap-analysis-2026-08-07.md`): 144 requirements, 79% fully covered, 5 critical gaps. Frontend: +3 gap scenarios → **264** vitest tests. 121 new backend unit tests across 6 modules (api_client, main, indexes, embedder, atlas_storage, rate_limiter); combined backend coverage raised 59.1% → **70.1%** (**468** pytest); floors updated 72/59/72 (stmts/br/lines); `--cov-fail-under` raised 59 → 70. Embedder empty-input `IndexError` fixed. **IMPLEMENTED**; **VERIFIED** locally (pre-push gates green). |
...
|------|-------|----------|-----|
| 2026-09-11 | hygiene | Split GHA cadence like quine-factory: ultra-minimal PR CI; nightly/supply-chain/mutation separate files; live DB + Docker off PR path | PR feedback speed; expensive jobs belong on less frequent slots; human-approved topology change (DECISION-OWNERSHIP gate topology) |
...
| 2026-09-10 | chore | Nightly T4 → three-bucket cadence (A/B/C) | **SUPERSEDED** 2026-09-11 by separate workflow files (quine-factory cadence). Was: monolithic `nightly.yml` A/B/C crons. Now: `nightly.yml` daily · `supply-chain.yml` Mon 03:00 · `mutation.yml` 1st+15th. |
