...
**Current** (2026-09-24 Add): new embedding-provider track **48A → 48B → 48C** (+48D Could; DoubleWord Qwen3-Embedding-8B, batch-first; brief [`BRIEF-doubleword-embedder.md`](../BRIEF-doubleword-embedder.md); DECISIONS #188–#212, reviewer fixes #234–#237, owner D2/D3 decisions #238–#239, ADR-005 Proposed 2026-09-25) — 48A is the next startable Must, starting with its T0 spike (#212), alongside **28** (external). Also added 2026-09-24 (docs only): Elasticsearch vector-store track **49 → 50 → 51** (#213–#219) and Redis track **52 → 53** (+54 Could; brief [`BRIEF-redis-evaluation.md`](../BRIEF-redis-evaluation.md); #220–#229). **52** is docs-only and may start now; **53** waits on 51 ✅ + 52 GO. Previous: Next Must **28** (external). **22** ✅ · **40** ✅ · **45** ✅ · **44** ✅ Residual §4 **IMPLEMENTED** (#163; Nightly artifact VERIFIED pending). Migration track: **32/33/34–38 code ✅ on main**; formal gate-closure debt now scoped to **32C** → **32B** only (coverage/mutation/nw-review + gate-evidence schema — see reconcile 2026-09-23, DECISIONS #186–#187). Then **41B**. Deferred Mongo QoL **26/27/19**. Genuinely-open craft: **46** (backend cov 80–85%), **47** (xenon B/A/A).
...
| 49 — Vector-store port split + registry | 📋 PLANNED | ~4–6 h | **Must** — `VectorStoreAdapter` protocol + `VectorCapabilities` + static registry (lazy import); relocates `== "mongodb"/"postgres"` branching out of health/guard/main into adapters; adds `VECTOR_STORE_BACKEND` (D1, defaults to `STORAGE_BACKEND`) + D2 guard. **No ES code, zero behaviour change.** — [`SLICE-49-VECTOR-STORE-PORT-SPLIT-REGISTRY.md`](05-storage/SLICE-49-VECTOR-STORE-PORT-SPLIT-REGISTRY.md) |
