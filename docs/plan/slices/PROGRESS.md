...
| 2026-08-07 | ATDD test suite audit + gap analysis (chore/project-hygiene-full) | nw-acceptance-designer-reviewer produced requirements-based gap analysis (`docs/plan/gate-evidence/test-gap-analysis-2026-08-07.md`): 144 requirements, 79% fully covered, 5 critical gaps (phase timeout, per-phase error handling, text-extraction edge cases, dashboard network resilience, state-transition validation). Frontend: all 24 files already ATDD-compliant; +3 gap scenarios → **264** vitest tests. Backend ATDD upgrade: `test_cli_print_summary.py`, `test_indexes_cmd.py`, `test_check_backend_coverage_floors.py`, `test_check_coverage_threshold_drift.py` upgraded with GWT markers, module docstrings, domain-language scenario names. New unit tests for 6 uncovered modules (api_client, main, indexes, embedder, atlas_storage, rate_limiter) in progress. **IMPLEMENTED** (partial — backend ATDD upgrade of remaining 40+ files deferred). |
