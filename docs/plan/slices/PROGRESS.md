...
**Current** (2026-09-24 Add): new embedding-provider track **48A → 48B → 48C** (+48D Could; DoubleWord Qwen3-Embedding-8B, batch-first; brief [`BRIEF-doubleword-embedder.md`](../BRIEF-doubleword-embedder.md); DECISIONS #188–#206) — 48A is the next startable Must alongside **28** (external). Previous: Next Must **28** (external). **22** ✅ · **40** ✅ · **45** ✅ · **44** ✅ Residual §4 **IMPLEMENTED** (#163; Nightly artifact VERIFIED pending). Migration track: **32/33/34–38 code ✅ on main**; formal gate-closure debt now scoped to **32C** → **32B** only (coverage/mutation/nw-review + gate-evidence schema — see reconcile 2026-09-23, DECISIONS #186–#187). Then **41B**. Deferred Mongo QoL **26/27/19**. Genuinely-open craft: **46** (backend cov 80–85%), **47** (xenon B/A/A).
...
| 48C — MRL dims + query-instruction axes | 📋 PLANNED | ~4–6 h | **Should** — one paid pass at max dim + truncate; instruction variants reuse doc vectors; **HITL D1–D3** (config shape, PG dims — HNSW ≤2000, identity) — [`SLICE-48C-DOUBLEWORD-MRL-INSTRUCTION-AXES.md`](08-embedding-providers/SLICE-48C-DOUBLEWORD-MRL-INSTRUCTION-AXES.md) |
