...
| Date | Item | Outcome |
|------|------|---------|
| 2026-09-10 | Nightly T4 three-bucket cadence (chore/project-hygiene) | Aligned `nightly.yml` to A/B/C: daily light T4, Monday supply chain, 1st+15th mutation; job-level `if:` on `github.event.schedule`. Docs: `development.md` + CHANGELOG. **IMPLEMENTED**; schedule **VERIFIED** pending next cron/`workflow_dispatch`. |
...
| 2026-09-10 | chore | Nightly T4 → three-bucket cadence (A/B/C) | **SUPERSEDES** single daily cron for all T4 jobs. **A** `0 2 * * *` light (tests, complexity, TruffleHog, dep-audit, gitleaks); **B** `0 2 * * 1` supply chain (SBOM, Meterian, container, chalk); **C** `0 2 1,15 * *` mutation (mutmut advisory + Stryker). Matches project-hygiene `ci-nightly-mixed` template; reduces Monday/mutation cost without dropping daily coverage signal. |
