...
| 48B — DoubleWord batch + cache + pre-embed + cost | 📋 PLANNED | ~6–8 h | **Must** — SQLite embedding cache shared across sweeps; batch client w/ checkpoint resume-on-resubmit; pre-embed declared space (grid **and** Bayesian); per-run + per-experiment cost; ADR-005 Accepted — [`SLICE-48B-DOUBLEWORD-BATCH-CACHE-PREEMBED.md`](08-embedding-providers/SLICE-48B-DOUBLEWORD-BATCH-CACHE-PREEMBED.md) |
