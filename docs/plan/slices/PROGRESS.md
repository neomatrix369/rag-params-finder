...
**Last Updated**: 2026-07-24 (nw-review fixes: action pinning, Stryker runner, pre-push fallback; frontend JUnit/coverage)
...
| 2026-07-24 | chore | T4 tier: nightly.yml (mutmut, Stryker, TruffleHog-full, SBOM, Meterian, container scan) | Deep checks too slow for CI (~15–60 min); scheduled weekly (Monday 02:00 UTC) + `workflow_dispatch` for manual runs; Meterian added as separate job (SCA + license, API-key-gated); container-scan skips if no Dockerfile (step-output guard, not job-level hashFiles — actionlint requirement); Trivy SBOM scan now `exit-code: 1` (consistent gate with container scan) |
