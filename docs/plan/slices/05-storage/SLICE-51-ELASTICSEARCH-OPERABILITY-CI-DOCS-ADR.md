...
- **One nightly matrix job (#248):** `vector-store-integration` in `nightly.yml`, matrix over `{mongodb, postgres, elasticsearch}` (Redis joins in 53), each leg running the store + vector contract suites and the 49B split-store acceptance test where the store is vector-only. "Skipped ≠ green" means two things. (1) Every leg sets `RAG_REQUIRE_<STORE>=1`, so live tests that would otherwise skip when the store is unreachable **fail** instead. (2) A final `if: always()` summary job fails unless every matrix leg's conclusion is `success`, so a cancelled or skipped leg also turns the nightly red. The existing `postgres-integration` and `mongo-integration` jobs (`nightly.yml:139-219`) fold into it.
