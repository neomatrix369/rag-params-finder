...
- [ ] Manual: submit identical local sweep with `parallelism: 1`, then `parallelism: 4`, capture elapsed/ETA difference from dashboard cards *(BLOCKED: host HTTP endpoint unavailable and `docker exec`/`docker compose` currently fail with Docker API permission errors: `permission denied while trying to connect to the docker API at unix:///Users/Mani/.docker/run/docker.sock`)*
