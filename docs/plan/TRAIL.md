...
| 49A | [../plan/slices/05-storage/SLICE-49-VECTOR-STORE-PORT-SPLIT-REGISTRY.md](../plan/slices/05-storage/SLICE-49-VECTOR-STORE-PORT-SPLIT-REGISTRY.md) | Vector-store port + registry (composites) + `get_vector_store()`; retriever reads the vector store; stores locked equal; characterization tests + AST store-string guard (no ES code, zero behaviour change) | Must | 📋 PLANNED | 32, 38 | — | ~4–6 h | 2026-09-25 |
| 49B | [../plan/slices/05-storage/SLICE-49B-VECTOR-STORE-DATA-PATH-REWIRE.md](../plan/slices/05-storage/SLICE-49B-VECTOR-STORE-DATA-PATH-REWIRE.md) | Rewire the 10 chunk data-path sites onto the vector port; two-store delete/stats/`/healthz`/boot/preflight; in-memory split-store E2E test; pairing rule (ii) | Must | 📋 PLANNED | 49A | — | ~6–8 h | 2026-09-25 |
...
| 2026-09-25 | nw-solution-architect-reviewer (48A + ADR-005, rounds 1–2) | **CONDITIONALLY APPROVED** → remediated | ADR-005 created (Proposed, T0 table pending); `defer_for_pre_embed` → `defer_async_embeddings` (#234); mixed-provider axis in ADR-005; `EmbeddingStrategy` escape hatch (#235) |
| 2026-09-25 | nw-system-designer-reviewer (48A, rounds 1–2) | **NEEDS REVISION** → **APPROVED** | SQLite shared-connection/WAL concurrency AC; `DOUBLEWORD_POLL_TIMEOUT_S` per-poll timeout; polling-interval rationale + backoff reset; unavailable-model reset path |
| 2026-09-25 | nw-data-engineer-reviewer (48C, rounds 1–2) | **REJECTED** → **CONDITIONALLY APPROVED** | D2 (no `embedding_512` column) + D3 (identity lineage) recorded as HITL blockers #236/#237 in Before-Checks; migration must merge before 48C branches; allowlist-guard After-Check added |
| 2026-09-25 | nw-documentarist-reviewer (48A docs AC, rounds 1–2) | **NEEDS REVISION** → **APPROVED** | Single-path setup guide; 1h/24h latency table; restart-safety vs cache-loss warning; async-provider section in `extending.md`; README `waiting` callout |
